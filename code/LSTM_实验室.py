...
def LSTM_stock(stock, mem_day):
    ...
    for i in range(len(stock)):
        x.append(stock.iloc[i, 9:15])
    x = np.array(x)

    # 创建输出集 y
    y = stock['S_DQ_CLOSE'].values
    print(len(x), ' ', x[:2], ' ', len(y))
...
opt_model(lstm_layers, dense_layers, units)